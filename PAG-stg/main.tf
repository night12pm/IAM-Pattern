...
resource "aws_iam_role_policy_attachment" "aws_managed_policy-4" {
  ...
  role       = "ppv_svcrole_stg_tenant${var.tenant_number}${var.patches[count.index]}"
...
resource "aws_iam_role_policy_attachment" "route53_manage_private_hz" {
  ...
  depends_on = [aws_iam_role_policy_attachment.ppv_kms_copy_s3_encrypted_objects-CMK]
}
...
resource "aws_iam_role_policy_attachment" "stg_ec2_autoscaling_access" {
...
resource "aws_iam_role_policy_attachment" "byok_kms_key_access" {
  ...
  depends_on = [aws_iam_role_policy_attachment.stg_ec2_autoscaling_access]
}
