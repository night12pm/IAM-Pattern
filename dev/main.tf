...
resource "aws_iam_role" "pag" {
  ...
  name  = "ppv_svcrole_${var.dev-tst-dem-int}_tenant${var.tenant_number}${var.all_roles[count.index]}"
...
resource "aws_iam_role_policy_attachment" "ppv_npn_ssm_policy" {
  count      = "${length(var.all_roles)}"
  role       = "ppv_svcrole_${var.dev-tst-dem-int}_tenant${var.tenant_number}${var.all_roles[count.index]}"
...
resource "aws_iam_role_policy_attachment" "route53_manage_private_hz" {
  ...
  role       = "ppv_svcrole_${var.dev-tst-dem-int}_tenant${var.tenant_number}${var.route53[count.index]}"
...
resource "aws_iam_role_policy_attachment" "s3_common_ro" {
  ...
  role       = "ppv_svcrole_${var.dev-tst-dem-int}_tenant${var.tenant_number}${var.s3_common[count.index]}"
...
resource "aws_iam_role_policy_attachment" "stg-ec2_autoscaling_access" {
  ...
  role       = "ppv_svcrole_${var.dev-tst-dem-int}_tenant${var.tenant_number}${var.dev_ec2[count.index]}"
...
# resource "aws_s3_bucket" "pag-terraform-tfstate" {
#     bucket = "pag-terraform-tfstate"
#     acl    = "private"
# }
